...
export function Dashboard() {
  ...
  const fetchDashboardSummary = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/dashboard/summary?month=${selectedMonth}&year=${selectedYear}`);
      if (!response.ok) {
  ...
  const fetchTodaysBookings = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/dashboard/todays-bookings');
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const data = await response.json();
