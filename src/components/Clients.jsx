...
export function Clients() {
  ...
  const handleDeleteClient = async (clientId) => {
    if (window.confirm('Are you sure you want to delete this client?')) {
      try {
        const response = await fetch(`https://111-production-573e.up.railway.app/api/clients/${clientId}`, {
          method: 'DELETE',
        });
        if (!response.ok) {
          const errorData = await response.json();
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => handleDeleteClient(client.id)} // Add delete button
                  >
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex justify-center items-center">
          <div className="bg-white p-8 rounded-lg shadow-xl max-w-md w-full relative">
            ...
            {selectedClient.companyId && (
              <div className="mb-4">
                <Label htmlFor="invoiceMonth" className="block text-sm font-medium text-gray-700 mb-1">Month</Label>
                <select
                  id="invoiceMonth"
            ...
            {selectedClient.companyId && (
              <div className="mb-6">
                <Label htmlFor="invoiceYear" className="block text-sm font-medium text-gray-700 mb-1">Year</Label>
                <Input
                  id="invoiceYear"
                  type="number"
            ...
            <div className="flex justify-end gap-3">
              ...
              <Button onClick={() => generateClientInvoice(selectedClient.id)}>
                Generate Invoice
              </Button>
            </div>
          </div>
